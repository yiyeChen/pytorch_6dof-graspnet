...
from graspnet_6dof.utils import sample

ROOT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
GRIPPER_PC = np.load(
                    os.path.join(ROOT_DIR, 'gripper_models/panda_pc.npy'),
...
def get_control_point_tensor(batch_size, use_torch=True, device="cpu"):
    ...
    """
    control_points = np.load(os.path.join(ROOT_DIR, './gripper_control_points/panda.npy'))[:, :3]
